...
export class ForoComponent  implements OnInit {
  ...
  guardarPublicacion() {
    if (this.publicacion.titulo.trim() === '') {
      showAlertDUOC('Debe agregar título');
      return;
    }
    if (this.publicacion.contenido.trim() === '') {
      showAlertDUOC('Debe agregar contenido');
      return;
    }
  ...
  editarPublicacion(pub: any) {
    if (pub.correo !== this.usuario.correo) {
      showAlertDUOC('Sólo puede editar las publicaciones propias');
      return;
    }
  ...
  mensajePublicacion(accion: string, id: Publicacion) {
    showAlertDUOC(`La publicación ${id} fue ${accion} exitosamente`);
    this.limpiarPublicacion();
  }
  ...
  crearPublicacion() {
    ...
    this.api.crearPublicacion(this.publicacion).subscribe({
      ...
      error: (error) => showToast('El servicio no está disponible')
    });
  }
  ...
  eliminarPublicacion(pub: any) {
    if (pub.correo !== this.usuario.correo) {
      showAlertDUOC('Sólo puede eliminar las publicaciones propias');
      return;
    }
